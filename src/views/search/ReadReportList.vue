...
export default {
  ...
  mounted () {
    ...
    if (parseInt(this.flag) === 2) {
      this.getList('statement/overdueNoReadReports', { ids: this.ids, roleType: this.roleType })
    } else if (parseInt(this.flag) === 3) {
      this.getList('statement/overdueReadReports', { ids: this.ids, roleType: this.roleType })
    } else {
