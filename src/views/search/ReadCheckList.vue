...
export default {
  ...
  mounted () {
    ...
    if (parseInt(this.flag) === 2) {
      this.getList('statement/overdueChecks', { ids: this.ids, roleType: this.roleType })
    } else if (parseInt(this.flag) === 3) {
      this.getList('statement/overdueReadChecks', { ids: this.ids, roleType: this.roleType })
    } else {
      this.getList('statement/supervisionCheck', { ids: this.ids })
    }
  },
  computed: {
  },
