      ...
      >
        ...
        <md-field style="white-space: nowrap">
          <div>
            ...
            <md-field-item solid title="共检查表" align="right" :content="allCheck+'个'" arrow @click="toList(1)"/>
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      selectorValue: sessionStorage.getItem('jianCheckSelect')
    }
  },
  created () {
    this.startDate = this.getStartDate()
    this.endDate = this.getEndDate()
    this.selectorValue = sessionStorage.getItem('jianCheckSelect') ? sessionStorage.getItem('jianCheckSelect') : '执行人'
  ...
  methods: {
    getData () {
      ...
      let qs = this.$qs
      this.$http.get('/statement/queryCheckRead', {
        params: {
    ...
    toList (val) {
      ...
      switch (val) {
        case 1: {
          this.$router.push({ name: 'readCheckList', query: { ids: this.allCheckIds, roleType: roleT, flag: 0, title: '共检查表' } })
          break
        }
        case 2: {
          this.$router.push({ name: 'readCheckList', query: { ids: this.normalCheckIds, roleType: roleT, flag: 1, title: '正常完成阅读' } })
          break
        }
        case 3: {
          this.$router.push({ name: 'readCheckList', query: { ids: this.overdueCheckIds, roleType: roleT, flag: 2, title: '逾期未阅读' } })
          break
        }
        case 4: {
          this.$router.push({ name: 'readCheckList', query: { ids: this.overdueDisposeCheckIds, roleType: roleT, flag: 3, title: '逾期已阅读' } })
          break
        }
      }
    },
  ...
  watch: {
    '$store.state.selectUserArrCheck': {
    ...
    selectorValue (newVal, oldVal) {
      sessionStorage.setItem('jianCheckSelect', newVal)
      if (!oldVal) {
        return
      }
      this.selectUser = [all]
